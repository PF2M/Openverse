...
echo '<div class="main-column"><div class="post-list-outline"><h2 class="label">' . htmlspecialchars($row['user_name']) . '\'s Followers</h2>';
if($can_see == true) {
echo '<div class="list follow-list">';
$fsql = 'SELECT * FROM follows LEFT JOIN users ON user_pid = follow_by WHERE follow_to = ' . mysqli_real_escape_string($link, $row['user_pid']) . ' ORDER BY follow_id DESC';
...
echo '<div class="no-content"><p>This user currently has no followers.</p></div>';
}} else {
